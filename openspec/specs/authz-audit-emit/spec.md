...
Define how `pkg/authz` integrates with `pkg/audit` to emit authorization decision events. The middleware SHALL support optional audit recorder injection and automatically emit `authz.decision` events after each authorization check, enabling observability without requiring caller-side instrumentation.
