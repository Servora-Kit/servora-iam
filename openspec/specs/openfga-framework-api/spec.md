...
Define the framework-level API contract for `pkg/openfga`, ensuring all methods accept generic principals (not hardcoded user types), support functional option configuration, and provide cache-hit observability. This enables `pkg/openfga` to serve as a reusable framework component for any OpenFGA-based authorization model.
